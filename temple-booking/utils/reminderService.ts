...
export async function sendVastraSevaReminder(booking: any) {
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      ...
      <div style="background-color: #f5f5f5; padding: 20px; border-radius: 5px; margin: 20px 0;">
        ...
        <div style="margin: 20px 0; padding: 15px; background-color: #fff; border-radius: 5px;">
          ...
          ${
            booking.specialDay
              ? "<p><strong>Special Day Service</strong></p>"
              : ""
          }
        </div>
  ...
  return await transporter.sendMail({
    ...
    to: booking.email,
    subject: `Reminder: Upcoming Vastra Seva Service - ${format(
      new Date(booking.date),
      "MMM do, yyyy"
    )}`,
    html: emailContent,
...
export async function sendSevaReminder(booking: any) {
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      ...
      <div style="background-color: #f5f5f5; padding: 20px; border-radius: 5px; margin: 20px 0;">
        ...
        <div style="margin: 20px 0; padding: 15px; background-color: #fff; border-radius: 5px;">
          ...
          <p><strong>Date:</strong> ${format(
            new Date(booking.serviceDate),
            "EEEE, MMMM do, yyyy"
          )}</p>
          ${
            booking.timeWindow
              ? `<p><strong>Time:</strong> ${booking.timeWindow}</p>`
              : ""
          }
  ...
  return await transporter.sendMail({
    from: process.env.SMTP_FROM_EMAIL,
    to: booking.email,
    subject: `Reminder: Upcoming Seva Service - ${format(
      new Date(booking.serviceDate),
      "MMM do, yyyy"
    )}`,
    html: emailContent,
  ...
  try {
    ...
    for (const booking of vastraSevaBookings) {
      try {
        ...
        console.error(
          `Failed to send reminder for Vastra Seva booking ${booking.id}:`,
          error
        );
      }
    }
    ...
    for (const booking of sevaBookings) {
      try {
        ...
        console.error(
          `Failed to send reminder for Seva booking ${booking.id}:`,
          error
        );
      }
    }
